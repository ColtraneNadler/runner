...
let instructionsMobileElement = document.getElementById('instructions-mobile');
let instructionsDesktopElement = document.getElementById('instructions-desktop');
...
function startGame() {
	...
	.then(res => res.json())
	.then(res => {
		let cookie = getCookie('p2.instructions');

		if(!cookie) {
			instructionsElement.hidden = false;

			// which instructions to shiw?
			if(is_mobile)
				instructionsMobileElement.hidden = false;
			else
				instructionsDesktopElement.hidden = false;


			document.cookie = 'p2.instructions=true';
